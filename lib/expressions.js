...
function ModuleExpression(name, body) {
    this.evaluate = function (context) {
        const newcontext = contexts.createContext(context);
        const module = new Module(name, newcontext);
...
function CallExpression(expr, args) {
    this.evaluate = function (context) {
        const argvalues = args.evaluate(context);
        const fn = expr.evaluate(context);
...
function ForExpression(name, valuesexpr, expr) {
    this.evaluate = function (context) {
        const newcontext = contexts.createContext(context);
        const values = valuesexpr.evaluate(context);
        let result = null;
...
module.exports = {
	...
	loop: function (expr) { return new LoopExpression(expr); },
	return: function (expr) { return new ReturnExpression(expr); },
