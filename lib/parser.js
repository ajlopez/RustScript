...
var ConstantExpression = expressions. ConstantExpression,
    QualifiedNameExpression = expressions.QualifiedNameExpression,
    StructExpression = expressions.StructExpression,
    EnumExpression = expressions.EnumExpression,
    ArrayExpression = expressions.ArrayExpression,
    TupleExpression = expressions.TupleExpression,
    MatchExpression = expressions.MatchExpression,
    PatternExpression = expressions.PatternExpression,
    NameExpression = expressions.NameExpression,
    AssignVariableExpression = expressions.AssignVariableExpression,
    LetExpression = expressions.LetExpression,
...
var rules = [
    ...
    // Integer
    get('0-9').oneOrMore().and(get('i').zeroOrMore()).generate('Integer', function (value) { return new ConstantExpression(parseInt(value)) }),
    ...
    get('_').generate('Name', function (value) { return new NameExpression(value) }),
    ...
    // Function
    get('fn', 'Name', 'LeftPar', 'ParameterList', 'RightPar', 'RightArrow', 'LeftHandlebar', 'CompositeExpression', 'RightHandlebar').generate('Function',
        function (values) { return new FunctionExpression(values[1].getName(), values[3], values[7]); }),
    // TODO Review
    get('pub').generate('Pub'),
    get('Pub', 'fn', 'Name', 'LeftPar', 'ParameterList', 'RightPar', 'RightArrow', 'LeftHandlebar', 'CompositeExpression', 'RightHandlebar').generate('Function',
        function (values) { return new FunctionExpression(values[2].getName(), values[4], values[8], { public: true }); }),
    ...
    // Match
    get('match', 'Expression', 'LeftHandlebar', 'PatternList', 'RightHandlebar').generate('MatchExpression', function (values) { return new MatchExpression(values[1], values[3]); }),

    // Struct, Enum
    get('struct', 'Name', 'LeftHandlebar', 'NameList', 'RightHandlebar').generate('StructExpression', function (values) { return new StructExpression(values[1].getName(), values[3]); }),
    get('enum', 'Name', 'LeftHandlebar', 'NameList', 'RightHandlebar').generate('EnumExpression', function (values) { return new EnumExpression(values[1].getName(), values[3]); }),
    ...
    // Pattern
    get('Expression', 'RightFatArrow', 'Expression').generate('Pattern', function (values) { return new PatternExpression(values[0], values[2]); }),
    ...
    get('LeftBracket', 'ArgumentList', 'RightBracket').generate('Term', function (values) { return new ArrayExpression(values[1]); }),
    get('LeftPar', 'ArgumentList', 'RightPar').generate('Term', function (values) { return new TupleExpression(values[1]); }),
