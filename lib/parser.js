...
function LetExpression(name, expr) {
    this.evaluate = function (context) {
        var value = expr.evaluate(context);
        context.setLocalValue(name, value);
        return value;
...
var rules = [
    ...
    // Let
    get('let', 'Name', 'Assign', 'Expression').generate('Expression', function (values) { return new LetExpression(values[1].getName(), values[3]); }),
    ...
    // While
    get('while', 'Expression', 'LeftHandlebar', 'CompositeExpression', 'RightHandlebar').generate('Expression', function (values) { return new WhileExpression(values[1], values[3]); }),
        
    // Loop
    get('loop', 'LeftHandlebar', 'CompositeExpression', 'RightHandlebar').generate('Expression', function (values) { return new LoopExpression(values[2]); }),
        
    // If
    get('IfExpression').generate('Expression'),
